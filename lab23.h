...
void Unit::equip (Equipment *Wea)
{
	...
	{
		eq_old = equipment->getStat();
	}
	vector<int> eq_new = Wea->getStat();
	atk+=(eq_new[1]-eq_old[1]);
	def+=(eq_new[2]-eq_old[2]);
	int temp=hpmax;
	hpmax+=(eq_new[0]-eq_old[0]);
	if(temp>hpmax&&hp>hpmax)
	{
		hp=hpmax;
